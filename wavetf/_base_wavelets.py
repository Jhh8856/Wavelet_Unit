...
class DirWaveLayer2D(keras.layers.Layer):
    ...
    def call(self, batch):
        ...
        #if (self.bs is None) : self.bs = -1
        #if (self.bs is None): self.bs = 16
        if (self.bs is None): self.bs = 4 #必須與batch_size一致
...
class DirWaveLayer3D(keras.layers.Layer):
    """Abstract class with general methods for 2D wavelet transforms"""
    # in : (b, x, y, c) --> out: (b, nx, ny, 4*c)
    def call(self, batch):
        """Call the direct 2D wavelet.
        ...
        """
        self.bs, self.ox, self.oy, self.oz, self.cn = batch.shape.as_list()
        #print(batch.shape.as_list())
        #if (self.bs is None) : self.bs = -1
        #if (self.bs is None): self.bs = 16
        if (self.bs is None): self.bs = 4 #必須與batch_size一致
        self.nx, self.ny, self.nz = map(lambda x: math.ceil(x / 2), [self.ox, self.oy, self.oz])
        self.qx, self.qy, self.qz = map(lambda x: math.ceil(x / 2), [self.nx, self.ny, self.nz])
        return self.kernel_function(batch)
    def compute_output_shape(self, input_shape):
        bs = input_shape[0]
        x = input_shape[1]
        ...
        h2 = math.ceil(y / 2)
        h3 = math.ceil(z / 2)
        out_shape = tf.TensorShape([bs, h1, h2, h3, 8*cn])
        return(out_shape)

class InvWaveLayer3D(keras.layers.Layer):
    """Abstract class with general methods for 2D inverse wavelet transforms"""
    # in : (b, x, y, 4*c) --> out: (b, 2*x, 2*y, c)
    def call(self, batch):
        """Call the inverse 2D wavelet
        ...
        """
        self.bs, self.nx, self.ny, self.nz, self.cn = batch.shape.as_list()
        #if (self.bs is None) : self.bs = -1
        if (self.bs is None): self.bs = 4#必須與batch_size一致
        self.cn = self.cn // 8
        self.ox = self.nx * 2
    ...
    def compute_output_shape(self, input_shape):
        ...
        nz = input_shape[3]
        cn = input_shape[4] // 8
        out_shape = tf.TensorShape([bs, 2*nx, 2*ny, 2*nz, cn])
        return(out_shape)